...
void tjrc_setHardware(void)
{
    ...
    /* 初步计算电压 */
    float VCC_3V3 = tjrc_vadc_getAnalog(ADC_3V3_Pin)* 3.33f * 2.0f / 4096;
    float VCC_5V0 = tjrc_vadc_getAnalog(ADC_5V0_Pin)* 3.33f * 2.0f / 4096;
    float VCC_BAT = tjrc_vadc_getAnalog(ADC_VBAT_Pin)* 3.33f * 4.0f / 4096;
...
void tjrc_setPeripherals(void)
{
    ...
    tjrc_st7735_dispStr612(0,0,(uint8_t*)"hello",RGB565_YELLOW);
    /* 初始化摄像头 */
    if(!tjrc_setMt9v03x())
    {
        /* 若初始化摄像头成功，则亮起红色LED */
