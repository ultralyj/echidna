...
#include "tjrc_imageProc_value.c"
//  //声明全局变量-------------------------------------------
//  //  图像
//  //      图像预处理
...
//uint8_t binimage_flag = 2;          //二值化标志，默认没有二值化
...
//
////当下边线大于该数，强制退出搜线
//uint8_t line_amount = 100;
//
////连续搜到边界次数，中间搜线开始条件
//uint8_t edge_count_amount = 3;
//
...
////      get_binImage(70);//获取二值化图像
////      tjrc_mt9v03x_displayImage(image, 70);
//        tjrc_st7735_dispbin(image, 120, 80, 4, 28, 0xFFFF,threshold);
...
//            tjrc_st7735_dispStr612(5,134,"zheng shi zi",0xFFFF);
...
//            tjrc_st7735_dispStr612(5,134,"zhong shi zi",0xFFFF);
//            //edge_truncation(0);
...
//            tjrc_st7735_dispStr612(5,134,"zuo yuan huan",0xFFFF);
//            //printf("\n直线顶点为：(%d,%d),(%d,%d)",L_corner_col, L_corner_row, center_lost_corner_col_l, center_lost_corner_row_l);
//            line_left_top[0] = (float)(center_lost_corner_col_l - L_corner_col) / (float)(center_lost_corner_row_l - L_corner_row);   //k
//            line_left_top[1] = (float)(L_corner_col - line_left_top[0] * L_corner_row);  //b
//            //printf("  直线斜率：k=%f,b=%f\n", line_left_top[0], line_left_top[1]);
...
//            tjrc_st7735_dispStr612(5,134,"you yuan huan",0xFFFF);
//            line_right_top[0] = (float)(center_lost_corner_col_r - R_corner_col) / (float)(center_lost_corner_row_r - R_corner_row);   //k
//            line_right_top[1] = (float)(R_corner_col - line_right_top[0] * R_corner_row);  //b
//            //printf("  直线斜率：k=%f,b=%f\n", line_left_top[0], line_left_top[1]);
//            //显示该直线
...
//            return 0;
//    }
//    else if (tjrc_imageProc_ctr.binimage_flag == 1)
//    {
...
//        /* 计算不同方向梯度幅值  */
//        /* 90 deg  */
//        temp[0] = -(int)image[(row - 1)*MT9V03X_W + (col - 1)] + (int)image[(row - 1)*MT9V03X_W + (col + 1)]     // -1,  0,  1
//            - (int)image[(row)*MT9V03X_W + (col - 1)] + (int)image[(row)*MT9V03X_W + (col + 1)]                  // -1,  0,  1
//            - (int)image[(row + 1)*MT9V03X_W + (col - 1)] + (int)image[(row + 1)*MT9V03X_W + (col + 1)];         // -1,  0,  1
//        /* 0 deg  */
//        temp[1] = -(int)image[(row - 1)*MT9V03X_W + col - 1] + (int)image[(row + 1)*MT9V03X_W + col - 1]     // -1, -1, -1
//            - (int)image[(row - 1)*MT9V03X_W + col] + (int)image[(row + 1)*MT9V03X_W + col]                  //  0,  0,  0
//            - (int)image[(row - 1)*MT9V03X_W + col + 1] + (int)image[(row + 1)*MT9V03X_W + col + 1];         //  1,  1,  1
//
//        /* 45 deg  */
//        temp[2] = -(int)image[(row - 1)*MT9V03X_W + col] + (int)image[(row)*MT9V03X_W + col - 1]             //  0, -1, -1
//            - (int)image[(row)*MT9V03X_W + col + 1] + (int)image[(row + 1)*MT9V03X_W + col]                  //  1,  0, -1
//            - (int)image[(row - 1)*MT9V03X_W + col + 1] + (int)image[(row + 1)*MT9V03X_W + col - 1];         //  1,  1,  0
//        /* 135 deg  */
//        temp[3] = -(int)image[(row - 1)*MT9V03X_W + col] + (int)image[(row)*MT9V03X_W + col + 1]             // -1, -1,  0
//            - (int)image[(row)*MT9V03X_W + col - 1] + (int)image[(row + 1)*MT9V03X_W + col]                  // -1,  0,  1
//            - (int)image[(row - 1)*MT9V03X_W + col - 1] + (int)image[(row + 1)*MT9V03X_W + col + 1];         //  0,  1,  1
//
//        temp[0] = abs(temp[0]);
//        temp[1] = abs(temp[1]);
//        temp[2] = abs(temp[2]);
...
//        temp[3] = (int)image[(row - 1)*MT9V03X_W + col - 1] + (int)image[(row - 1)*MT9V03X_W + col] + (int)image[(row - 1)*MT9V03X_W + col + 1]
//            + (int)image[(row)*MT9V03X_W + col - 1] + (int)image[(row)*MT9V03X_W + col] + (int)image[(row)*MT9V03X_W + col + 1]
//            + (int)image[(row + 1)*MT9V03X_W + col - 1] + (int)image[(row + 1)*MT9V03X_W + col] + (int)image[(row + 1)*MT9V03X_W + col + 1];
//
