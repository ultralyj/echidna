/**
 * @file tjrc_fileIO.c
 * @author YYYDS team (1951578@tongji.edu.cn)
 * @brief 文件读写相关算法
 * @version 0.1
 * @date 2022-03-13
 * 
 * @copyright Copyright (c) 2022
 * 
 */

#include <tjrc_mt9v03x.h>
#include "tjrc_fileIO.h"

/**
 * @brief 120*80 8bit灰度bmp文件头，节省空间，直接固定
 *
 */
const uint8_t bmp_header_120x80_gray[]={
    0x42, 0x4D, 0xB8, 0x29, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x36, 0x04, 0x00, 0x00, 0x28, 0x00,
    0x00, 0x00, 0x78, 0x00, 0x00, 0x00, 0x50, 0x00, 0x00, 0x00, 0x01, 0x00, 0x08, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x82, 0x25, 0x00, 0x00, 0x3A, 0x09, 0x00, 0x00, 0x3A, 0x09, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00
};

/**
 * @brief 60*40 8bit灰度bmp文件头，节省空间，直接固定
 * 
 */
const uint8_t bmp_header_60x40_gray[]={
    0x42, 0x4D, 0x98, 0x0D, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x36, 0x04, 0x00, 0x00, 0x28, 0x00,
    0x00, 0x00, 0x3C, 0x00, 0x00, 0x00, 0x28, 0x00, 0x00, 0x00, 0x01, 0x00, 0x08, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x62, 0x09, 0x00, 0x00, 0x3A, 0x09, 0x00, 0x00, 0x3A, 0x09, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00
};

/**
 * @brief 调色板数据（可以通过改调色板，将特点灰度值映射到某种颜色，可以实现过曝死黑提示或者线提示）
 * 
 */
const uint8_t bmp_pannel_gray[]={
    
};

/**
 * @brief 将摄像头的图像转化为BMP文件存储与SD卡上
 * 
 * @param path 存储路径（绝对地址）
 * @param image 图像地址
 */
void tjrc_fileIo_camera2bmp(char* path, uint8_t *image)
{
    static FIL fil_bmp;
    FRESULT ff_res;

    uint32_t wcnt = 0;
    /* 图像部分一共要写入的字节数 */
    const uint32_t bcnt = MT9V03X_W*MT9V03X_H/4, block_size = 1200;

    rt_tick_t tcnt = rt_tick_get();

    /* 打开文件 */
    ff_res = f_open(&fil_bmp,path,FA_CREATE_NEW|FA_READ|FA_WRITE);
    /* 写入BMP文件头 */
    f_write(&fil_bmp, bmp_header_60x40_gray,sizeof(bmp_header_60x40_gray),&wcnt);
    /* 写入调色板信息 */
    f_write(&fil_bmp, bmp_pannel_gray,sizeof(bmp_pannel_gray),&wcnt);
    ff_res = f_sync(&fil_bmp);

    for (uint16_t i =0;i<bcnt/block_size;i++)
    {
        f_write(&fil_bmp, &image[i*block_size],block_size,&wcnt);
        ff_res = f_sync(&fil_bmp);
    }
    ff_res = f_close(&fil_bmp);
    printf("[ff]save: %s(%d) t:%d ms\r\n",path,ff_res,rt_tick_get() - tcnt);
}


